...
import { UserRound, Check, Lock, Loader2, Pencil, X, ChevronRight, Phone } from "lucide-react";
...
import { updateMyBasicProfile, changeMyStaffPassword } from "@/lib/actions/profile";
...
export function StaffProfileEditor({
  ...
  async function saveProfile() {
    setErr(null);
    setSaving(true);
    const res = await updateMyBasicProfile({ full_name: draftName, phone: draftPhone });
    setSaving(false);
    if (!res.ok) {
  ...
  async function savePassword() {
    ...
    setPwSaving(true);
    const res = await changeMyStaffPassword(pw);
    setPwSaving(false);
    if (!res.ok) {
        ...
        <CardBody className="space-y-5 pt-0">
          ...
          <div className="border-t border-border pt-4">
            <div className="mb-3 flex items-center justify-between">
              <h2 className="text-sm font-bold uppercase tracking-wide text-muted-2">Personal details</h2>
              {!editing && (
                <button
                  onClick={startEdit}
            ...
            {editing ? (
              <div className="space-y-4">
                ...
                {err && <p className="rounded-xl bg-emergency-soft px-3.5 py-2.5 text-sm font-medium text-emergency">{err}</p>}
          ...
          <div className="border-t border-border pt-4">
            ...
            >
              ...
              <ChevronRight size={18} className={`text-muted-2 transition-transform ${pwOpen ? "rotate-90" : ""}`} />
            </button>
            ...
            {pwOpen && (
              <div className="mt-4 space-y-4">
                ...
                {pwErr && <p className="rounded-xl bg-emergency-soft px-3.5 py-2.5 text-sm font-medium text-emergency">{pwErr}</p>}
                {pwMsg && <p className="rounded-xl bg-success-soft px-3.5 py-2.5 text-sm font-medium text-success">{pwMsg}</p>}
