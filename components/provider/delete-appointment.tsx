...
import { deleteProviderAppointment } from "@/app/provider/appointments/actions";

export function ProviderDeleteAppointment({ id }: { id: string }) {
  ...
  async function remove() {
    setLoading(true);
    setError(null);
    const res = await deleteProviderAppointment(id);
    setLoading(false);
    if (!res.ok) {
        ...
        <div className="fixed inset-0 z-[120] flex items-center justify-center bg-black/60 p-4">
          <div className="w-full max-w-sm rounded-2xl border border-border bg-surface p-6 text-center shadow-float">
            ...
            <p className="mt-1.5 text-sm text-muted">
              This permanently removes the appointment. This can’t be undone.
            </p>
