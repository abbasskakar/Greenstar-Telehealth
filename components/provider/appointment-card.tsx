...
export function ProviderAppointmentCard({ appt }: { appt: AppointmentCardData }) {
  ...
  const timeStr = when
    ...
    : null;
  const meta2 = [p?.age != null ? `${p.age} yrs` : null, p?.gender, appt.specialty]
    .filter(Boolean)
    .join(" · ");
      ...
      <div className="flex items-start gap-3 p-4">
        ...
        <div className="flex min-w-0 flex-1 items-start justify-between gap-2">
          <div className="min-w-0">
            ...
            <p className="truncate text-sm text-muted">{meta2 || appt.specialty}</p>
          </div>
